...
def show_board():
    '''
    Function to basically show the board.
    Prints new line 100 times to hide to old board after every time the function is called.
    '''
    print('\n'*100)
    print("""
████████╗██╗░█████╗░  ████████╗░█████╗░░█████╗░  ████████╗░█████╗░███████╗
...
def user_mark(marker, position):
    '''
    takes the marker and position choosen by the user as input.
    checks if position choosen by the user is'nt already pre-occupied
    if not , it marks that position with user's choosen marker (X or O)

    '''

    if position % 3 == 0:  # same thing happening as in comp_mark
        pos1 = 2
...
def win_check(board, mark):
    '''
    checks all the possible combinations which results in win for either the user or computer
    '''
